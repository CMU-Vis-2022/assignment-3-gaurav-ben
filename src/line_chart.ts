...
export function lineChart() {
    ...
    var base_layer = svg.append("g");
    var text_layer = svg.append("g");
    ...
    var m_box = text_layer.append("rect")
        .style("fill", "none")
    ...
    function togglePts() {
        // Update dots set
        var pts = base_layer.selectAll(".dots").data(raw_data);
        ...
        pts.enter()
            ...
            .attr("class", "dots")
            .merge(pts)
            .attr("cx", function(d: any) { return xScale(d.date) })
            .attr("cy", function(d: any) { return yScale(d.aqi) })
            .attr('r', 1.5)
    ...
    function update(X: string[],
        ...
        function mousemove() {
            ...
            m_text
                .html("<tspan dy='1.2em'>" + m_date + "-15" + "</tspan>"
                    + "<tspan dx='-5.2em' dy='1.2em'>" + "Mean US AQI: " 
                    + selectedData.aqi.toFixed(2) + "</tspan>")
                .attr("x", modifiedDate)
        ...
        base_layer.select<SVGElement>(".xaxis").call(xAxis)

        base_layer.select<SVGElement>(".yaxis").call(yAxis)
            // add gridlines
            .call((g) =>
